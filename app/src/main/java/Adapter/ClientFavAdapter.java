...
public class ClientFavAdapter extends RecyclerView.Adapter<ClientFavAdapter.ViewHolder> {
    ...
    public ClientFavAdapter(Context context, ArrayList<RequestsModel> faqModels) {
                ...
                .getAsJSONObject(new JSONObjectRequestListener(){
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            if (success.equals("1")){
                                holder.like.setImageResource(R.drawable.like);
                                Toast.makeText(context, "المنتج موجود بالفعل ف المفضلة", Toast.LENGTH_SHORT).show();

                            }
                            else if (success.equals("0")){
                                setInFav=false;
                                int adapterPosition = holder.getAdapterPosition();
                                faqModels.remove(adapterPosition);
                              notifyDataSetChanged();
                                holder.like.setImageResource(R.drawable.im1);
